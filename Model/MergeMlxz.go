...
func ModelMlxz() {
	...
	serverRemotePort := viper.GetString("config.serverRemotePort")
	...
	for _, fq := range fuquArr {
		...
		//INSERT INTO `game_center`.`t_merge_record`(`platform`, `fromServerId`, `toServerId`, `finalServerId`) VALUES (1, 2, 1, 1)
		INSERTIdSql := fmt.Sprintf("INSERT INTO `game_center`.`t_merge_record`(`platform`, `fromServerId`, `toServerId`, `finalServerId`) VALUES (1, '%d', '%d', '%d')", sSid, mSid, mSid)
		_, err = db.Exec(INSERTIdSql)
		if err != nil {
			logrus.Errorf("执行SQL失败: %v", errors.WithStack(err))
		} else {
			logrus.Info("插入合服ID成功")
		}
		UpdateIdSql := fmt.Sprintf("UPDATE `game_center`.`t_server` SET serverPort ='%s',serverRemotePort = '%s',serverDBName='%s'  where serverType=0 and serverId= %d", serverPort, serverRemotePort, zhuquId, sSid)
