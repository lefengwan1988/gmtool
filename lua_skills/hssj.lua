...
game_config = {
    ...
    -- 是否更新角色名称（添加服务器前缀）
    update_actor_name = true,

    -- 是否更新服务器索引
    update_server_index = true,
}

-- 排除表列表
exclude_tables = {
    "slgcityinfo", "cityquest", "cityfightlog", "useritem",
    "detailreportnew_1", "detailreportnew_2", "detailreportnew_3",
    "detailreportnew_4", "detailreportnew_5", "fightreportnew",
    "martialartsmatchinfo", "martialartsfightreportinfo",
    "fightreportgroupnew_1", "fightreportgroupnew_2", "fightreportgroupnew_3",
    "fightreportgroupnew_4", "fightreportgroupnew_5",
    "jobcount", "zycount", "nationlist", "nationrecord", "nationmember",
    "nationactorquest", "actormails", "jingjirank", "investbuycount",
    "towerrank", "toprank", "nationquest", "slgmaparmy",
    "martialartschampioninfo", "towerrecord"
}
...
function execute(config)
    ...
    for i, sub_server in ipairs(sub_servers) do
        ...
        -- 更新角色名称（如果配置启用）
        if game_config.update_actor_name then
            local update_name_sql = string.format(
                "UPDATE `%s`.`actors` SET actorname = CONCAT('s%s', actorname)",
        ...
        -- 更新服务器索引（如果配置启用）
        if game_config.update_server_index then
            local update_index_sql = string.format(
                "UPDATE `%s`.`actors` SET serverindex = %s WHERE serverindex = %s",
                sub_db, main_server_id, sub_server_id)
            db.exec(update_index_sql)
            log.debug("更新服务器索引完成")
        end
