...
game_config = {
    ...
    -- 是否插入合服记录
    insert_merge_record = true,

    -- 合服记录数据库
    merge_db = "game_center",

    -- 是否更新服务器配置
    update_server_config = true,

    -- 服务器配置数据库
    server_config_db = "game_center",

    -- 服务器端口配置（魔灵修真特有）
    serverPort = 4001,
    serverRemotePort = 21001,
}
...
function execute(config)
    ...
    for i, sub_server in ipairs(sub_servers) do
        ...
        -- 插入合服记录
        if game_config.insert_merge_record then
            local insert_sql = string.format(
                "INSERT INTO `%s`.`t_merge_record`(`platform`, `fromServerId`, `toServerId`, `finalServerId`) VALUES (1, %s, %s, %s)",
                game_config.merge_db, sub_server_id, main_server_id, main_server_id)
            local success, err = db.exec(insert_sql)
            if success then
                log.debug("插入合服记录成功")
            else
                log.warn("插入合服记录失败: " .. err)
            end
        end

        -- 更新服务器配置
        if game_config.update_server_config and config.server_port and config.server_remote_port then
            local update_sql = string.format(
                "UPDATE `%s`.`t_server` SET serverPort='%s', serverRemotePort='%s', serverDBName='%s' WHERE serverType=0 AND serverId=%s",
                game_config.server_config_db, config.server_port, config.server_remote_port, config.main_server, sub_server_id)
            local success, err = db.exec(update_sql)
            if success then
                log.debug("更新服务器配置成功")
            else
                log.warn("更新服务器配置失败: " .. err)
            end
        end
